          ...
          <div key={r.id||idx} className="p-3 bg-white/3 rounded">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                <button onClick={()=>startEdit(r)} className="p-2 bg-white/5 rounded">Edit</button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 flex items-end md:items-center justify-center p-4">
          <div className="w-full md:w-2/3 bg-bg p-4 rounded-lg border border-white/5">
            ...
            <div className="mt-2">
              ...
              <select value={editData.winnerId} onChange={e=>setEditData((d:any)=>({...d, winnerId:e.target.value, winnerName: game.players.find((p:any)=>p.id===e.target.value)?.name}))} className="w-full p-2 bg-white/5 rounded mt-1">
                {game.players.map((p:any)=> <option key={p.id} value={p.id}>{p.name}</option>)}
              </select>
            </div>
            <div className="mt-2">
              ...
              <div className="space-y-2 mt-2">
                {game.players.map((p:any)=> (
                  <div key={p.id} className="flex gap-2 items-center">
                    ...
                    <input type="number" value={editData.scores[p.id]} onChange={e=>changeScore(p.id, Number(e.target.value)||0)} className="flex-1 p-2 bg-white/5 rounded" />
                  </div>
                ))}
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              <button onClick={saveEdit} className="flex-1 p-3 bg-gold text-black rounded">Save</button>
              <button onClick={()=>{setEditingId(null); setEditData(null)}} className="p-3 bg-white/5 rounded">Cancel</button>
            </div>
          </div>
        </div>
      )}
