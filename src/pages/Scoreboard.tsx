  ...
  if (!game)
    return (
      <div>
        <p>
          No active game.{' '}
          <Link to="/new" className="text-gold">
            Create one
          </Link>
        </p>
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        {sorted.map(p => (
          <div
            key={p.id}
            className="p-3 rounded bg-white/3 flex justify-between items-center transition-transform transform hover:scale-[1.01]"
          >
            <div>
            ...
            <div className="text-right">
              ...
              {loser && loser.id === p.id && (
                <div className="text-xs text-red-400">Reached 100+</div>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="flex gap-2">
        <button
          onClick={onNewRound}
          className="flex-1 btn-large bg-gold text-black rounded"
        >
          New Round
        </button>
        <button
      ...
      <div className="space-y-2">
        <Link to="/history" className="block p-3 bg-white/5 rounded">
          Round History
        </Link>
        <Link to="/stats" className="block p-3 bg-white/5 rounded">
          Statistics
        </Link>
        <Link to="/settings" className="block p-3 bg-white/5 rounded">
          Settings
        </Link>
      </div>
      ...
      {showEndDialog && (
        <div className="fixed inset-0 flex items-center justify-center modal-backdrop p-4">
          <div className="w-full max-w-md bg-bg p-4 rounded-lg border border-white/5">
            ...
            <p className="mt-2">
              A player reached 100 points and lost. Current winner (lowest points):{' '}
              <strong>{winner?.name}</strong> with {winner?.points} points.
            </p>
            <div className="flex gap-2 mt-4">
              <button
                ...
                className="flex-1 p-3 bg-white/5 rounded"
              >
                Continue
              </button>
              <button
